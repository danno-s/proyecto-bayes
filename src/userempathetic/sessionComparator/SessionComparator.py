...
def getRawSessionData(session_id, sessionTable='sessions'):
    sqlCD = sqlWrapper('CD')
    row = sqlCD.read("SELECT sequence,profile,inittime,endtime,user_id FROM "+sessionTable+" WHERE id = "+ str(session_id))
    return row[0]
...
if __name__ == '__main__':
    ...
    from src.userempathetic.metrics.sessionMetrics.TimeMetrics import DurationDifference
    durationdifference = sC.compareSessions(DurationDifference())
    print("DurationDifference = "+ str(durationdifference) + " seconds.")