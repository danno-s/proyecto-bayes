...
class Sessionizer:
    ...
    def extractSessionsOf(self, user_id, stepsGen):
        ...
        for step in stepsGen:
            ...
            if step[0] - prevStep[0] <= self.tlimit:   # condición para mantenerse en sesión actual
                ...
                if sb.first() == sb.at(1):
                    sb.remove(0)
                sessions.append(self.toSession(sb.dump(),profile,initTime,endTime,user_id))  # guardar sesión actual del usuario
            ...
            if sb.first() == sb.at(1):
                sb.remove(0)
            sessions.append(self.toSession(sb.dump(),profile,initTime,endTime, user_id))   # guardar última sesión del usuario.
    ...
    @abstractmethod
    def bufferAccepts(self,sb,prevStep,step):pass

    @abstractmethod
    def toIDPair(self, macro_id, micro_id): pass
